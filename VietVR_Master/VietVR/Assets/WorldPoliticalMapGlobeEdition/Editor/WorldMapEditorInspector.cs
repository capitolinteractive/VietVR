...
namespace WPM {
	...
	public class WorldMapEditorInspector : Editor {
		...
		public override void OnInspectorGUI () {
			...
			_editor.operationMode = (OPERATION_MODE)GUILayout.Toolbar ((int)_editor.operationMode, mainToolbarIcons, toolbarStyle, GUILayout.Height (24));
		...
		void RepositionOverlay () {
			_map.GetOverlayLayer (true).transform.position = new Vector3 (_map.transform.position.x + 5000f, 5000f, 0); // since it's already displaced, applying a zoom of 1000 will throw it into problematic coordinates due to floating point limitations
		}
		...
		void ShowEntitySelectors () {
			...
			if (_editor.countryIndex >= 0) {
				...
				if (_editor.cityIndex >= 0) {
					...
					if (GUILayout.Button ("Update")) {
						UndoPushCityStartOperation ("Undo Change Province");
						_editor.CityChangeProvince ();
						UndoPushCityEndOperation ();
					}
					EditorGUILayout.EndHorizontal ();
				}
